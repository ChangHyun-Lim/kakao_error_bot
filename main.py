...
def map_wtr(code: int):
    """
    3자리 WTR 표시용 에러코드를 -> 원본 에러코드(4자리 또는 음수)로 역변환
    """

    # ① 1000~1100 구간 역변환
    #    1000~1100 → 300~400 출력됨 (정방향: out = original - 700)
    #    역변환: original = code + 700
    if 300 <= code <= 400:
        return code + 700     # 301 → 1001, 372 → 1072

    # ② 2000~2100 구간 역변환
    #    2000~2100 → 400~500 (정방향: out = original - 1600)
    #    역변환: original = code + 1600
    if 400 <= code <= 500:
        return code + 1600    # 405 → 2005

    # ③ (-230 ~ -200) → (300~330) (정방향: out = (-code) + 300)
    #    역변환: original = -(code - 300)
    if 300 <= code <= 330:
        return -(code - 300)

    # ④ (-330 ~ -300) → (230~260) (정방향: out = (-code) + 230)
    if 230 <= code <= 260:
        return -(code - 230)

    # ⑤ (-530 ~ -500) → (60~100) (정방향: out = (-code) + 60)
    if 60 <= code <= 100:
        return -(code - 60)

    # ⑥ (-820 ~ -700) → ( -110~120 ) (정방향: out = (-code) - 110)
    #    out = (-code) -110 ⇒ (-code) = out +110 ⇒ code = -(out+110)
    if -110 <= code <= 120:
        return -(code + 110)

    # ⑦ (-1060 ~ -1000) → ( -? ) (정방향: out = (-code) - 290)
    if code >= 0 and code <= 60:
        return -(code + 290)

    # ⑧ (-1570 ~ -1500) → ( -? ) (정방향: out = (-code) - 730)
    if code >= 0 and code <= 70:
        return -(code + 730)

    # ⑨ (-1620 ~ -1600) → ( -? ) (정방향: out = (-code) - 760)
    if code >= 0 and code <= 20:
        return -(code + 760)

    # ⑩ (-1750 ~ -1700) → ( -? ) (정방향: out = (-code) - 840)
    if code >= 0 and code <= 50:
        return -(code + 840)

    # ⑪ (-3020 ~ -3000) → ( -? ) (정방향: out = (-code) - 2090)
    if code >= 0 and code <= 20:
        return -(code + 2090)

    # ⑫ (-3150 ~ -3100) → ( -? ) (정방향: out = (-code) - 2170)
    if code >= 0 and code <= 50:
        return -(code + 2170)
